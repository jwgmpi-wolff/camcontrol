...
def get_snapshot(camera_id: str, _: None = Depends(_require_api_key)) -> Response:
...
def list_media(camera_id: str, _: None = Depends(_require_api_key)) -> list[MediaFile]:
    backend = state.capture_backend_for(camera_id)
...
def download_media(
    camera_id: str, path: str, _: None = Depends(_require_api_key)
) -> StreamingResponse:
    backend = state.capture_backend_for(camera_id)
    if not isinstance(backend, MediaBrowser):
        raise HTTPException(
